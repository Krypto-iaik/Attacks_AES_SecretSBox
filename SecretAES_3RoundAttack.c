...
int foundKey(word8 key[][4])
{
    ...
    for(dddelta = 0; dddelta < 256; dddelta++)
    {
        ...
        for(k=1; k<NUMBER_CP; k++)
        {
            for(i=0; i<16; i++)
            {
                verify[i] = cipher[0][i] ^ cipher[k][i];
            }

            if((verify[2]!=0) || (verify[5]!=0) || (verify[8]!=0) || (verify[15]!=0))
            {
                flag = 1;
                break;
            }
...
The attack used is a truncated differential attack. Note: it is not necessary find any information of the S-Box to find the key! */
...
int main()
{
    ...
    printf("ATTACK on 3-rounds AES with SECRET S-Box.\n\n");
    printf("The program finds the secret key (actually 1 byte of it) without using any information about the S-Box.\n");
    printf("The attack used is a truncated differential attack. Note: it is not necessary find any information of the S-Box to find the key!\n\n");
